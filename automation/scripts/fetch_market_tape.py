...
already started. Everything is written as static JSON next to the page:
    ...
    webpage/research/markettape/data/schedule.json   {updated, watchlist, events:[...]}
    webpage/research/markettape/data/results.json    {updated, results:{event_id: {...}}}

The page itself makes no API calls — it renders these files and computes the
live windows and countdowns in the browser.

Usage:
    python automation/scripts/fetch_market_tape.py --out webpage/research/markettape/data
    python scripts/fetch_market_tape.py --out /tmp/tape --tickers NVDA AAPL --no-results
"""
...
def main() -> int:
    ...
    ap.add_argument("--out", default="webpage/research/markettape/data", help="output folder")
