...
└── research/               One folder per project, plus the index over them
    ...
    ├── markettape/         Market News page + Fed/earnings data (own README)
...
| `POST /api/admin/run/{stack,markettape,news,calendar,briefing}` | `worker/index.ts` | Runs a job now. Needs `Authorization: Bearer $ADMIN_TOKEN`. |
...
| 12:10 and 22:10, Mon–Fri | Fed events and earnings for the Market News calendar (the former Market Tape job), via Gemini + Google Search | `documents` |
| with those runs | Deletes contact messages 30 days after they were answered | `contact_messages` |
| every 15 min | Market News (`worker/news/`): headlines every run on weekdays and hourly at weekends; Gemini briefings at 02:30, 08:00, 12:30, 16:30 New York time on weekdays and Sat 10:00; the calendar and 7-day pruning at 05:00 New York time | `news_*`, `documents` |
