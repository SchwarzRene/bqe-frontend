...
looking. Lives at **https://schwarzrene.github.io/research/stack/** once these files are
in the repo.
...
```
webpage/research/stack/index.html            the page
webpage/research/stack/data/                 generated — one JSON per ticker + index.json
webpage/research/stack/live.example.json     template for the optional live proxy
automation/scripts/fetch_market_data.py      yfinance -> JSON
automation/requirements.txt                  yfinance, pandas, lxml
.github/workflows/market-data.yml            weekdays 22:20 UTC + manual run
automation/worker/                           optional Cloudflare Worker for live bars
```
...
file at it — see `automation/worker/README.md`. It exists because Yahoo sends no CORS headers, so the
browser needs something that will answer it; the Worker relays the same JSON
shape with the header attached.

With `live.json` present each card asks the Worker first and falls back
...
Jekyll, this folder is copied through untouched.
...
1. The four paths above are already in the repository (these setup notes live
   at `webpage/research/stack/README.md`; they shipped as `STACK-SETUP.md`).
...
3. Go to **Actions → Market data → Run workflow** and start it by hand once.
   ...
   the full index.
4. Give Pages a minute to rebuild, then open `/research/stack/`.
...
python automation/scripts/fetch_market_data.py \
  --out webpage/research/stack/data --tickers AAPL MSFT NVDA
cd webpage && python3 -m http.server 8000
# open http://localhost:8000/research/stack/
```

Opening `index.html` straight off the filesystem won't load the data —
