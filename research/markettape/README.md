...
| Calendar | Month or week grid, Google-Calendar style, events colored by category: Central banks, Speeches, Economic data, Earnings, Commodities (toggle each on or off) and filtered by importance (All, Notable+, Key only). A month cell shows the day's 3 most important events and "+n more"; busy days use the AI ranking and are marked "AI". Clicking a day opens the day panel: the AI's line on the day and the full schedule with results, streams and related headlines. Every event in the week view and the day panel has a ↗ button: its live stream while one is on, else the publisher's own page (Fed, ECB, BoE, SNB, BoJ, Bank of Russia, PBoC, EIA, USDA, OPEC, US Labor Department, NBS), the company's Yahoo page for earnings, or Yahoo's economic calendar for that day (consensus and actual) |
